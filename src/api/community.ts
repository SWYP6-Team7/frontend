import { Community, ICommunityList, Image, PostCommunity } from "./../model/community";
...
export async function getCommunities(accessToken: string | null, params: IListParams & { page: number }) {
  try {
...
export async function getMyCommunities(accessToken: string | null, params: IListParams & { page: number }) {
  try {
    const response = await axiosInstance.get(
      `/api/my-communities`,
...
export async function updateCommunity(data: PostCommunity, communityNumber: number, accessToken: string | null) {
  try {
    const contentData = {
    ...
    if (!accessToken) throw new Error(ERROR_MESSAGES.needLogin);
    const response = await axiosInstance.put(`/api/community/posts/${communityNumber}`, contentData, {
      headers: getJWTHeader(accessToken!),
    });
    return handleApiResponse(response) as Community;
  } catch (err: any) {
...
export async function updateImage(data: EditFinalImages, communityNumber: number, accessToken: string | null) {
  try {
    if (!accessToken) throw new Error(ERROR_MESSAGES.needLogin);
...
export async function postImage(data: FinalImages, communityNumber: number, accessToken: string | null) {
  try {
    if (!accessToken) throw new Error(ERROR_MESSAGES.needLogin);
