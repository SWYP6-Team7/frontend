...
export const useUploadStore = create<UploadState>(set => ({
  ...
  removeImage: (imageNumber: number) =>
    set(state => ({
      images: state.images.map(img =>
        img.imageNumber === imageNumber ? { ...img, isSelected: false } : img
      )
    })),

  saveFinalImages: () =>
    set(state => {
      const deletedTempUrls = state.images
        .filter(img => !img.isSelected)
        .map(img => img.url)
      const tempUrls = state.images
        .filter(img => img.isSelected)
        .map(img => img.url)
  ...
  reset: () =>
    set({ images: [], finalImages: { deletedTempUrls: [], tempUrls: [] } })
}))
...
export const useEditStore = create<EditState>(set => ({
  ...
  updateImage: (image: Image) =>
    set(state => {
      const existingImageIndex = state.images.findIndex(
        img => img.imageNumber === image.imageNumber
      )
  ...
  updateImageStatus: (imageNumber: number, status: 'y' | 'n' | 'd') =>
    set(state => ({
      images: state.images.map(img =>
        img.imageNumber === imageNumber ? { ...img, status } : img
      )
    })),
