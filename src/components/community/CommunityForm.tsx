...
import { EditFinalImages, EditImage, UploadImage, useEditStore, useUploadStore } from "@/store/client/imageStore";
...
const CommunityForm = ({ isEdit = false }: CommunityFormProps) => {
    ...
    if (isEdit) {
      ...
      const deletedImages = [
        ...
        ...initialImages.filter(
          (initialImg) => !currentImages.some((current) => current.imageNumber === initialImg.imageNumber)
        ),
      ];
  ...
  return (
    <>
      <Container>
        <Spacing size={8} />
        <Select initOpen={true} noneValue="말머리" list={LIST} value={value} setValue={changeValue} />
        <Spacing size={"3.8svh"} />
        <InputField
          value={title}
