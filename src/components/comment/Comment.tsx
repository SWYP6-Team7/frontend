...
const Comment = ({
  ...
  const { reportSuccess, setReportSuccess, setDetailId, setUserNumber } =
    reportStore();
    ...
    >
      <TopContainer>
        ...
        <UserBox onClick={() => moveToUserProfilePage(userNumber)}>
          {relatedType === "travel" &&
            comment.travelWriterNumber === comment.userNumber && (
              <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
...
const Container = styled.div<{
  ...
  background-color: ${(props) =>
    props.isEdit
      ? "rgba(227, 239, 217, 0.3)"
      : props.relatedType === "community"
        ? "#f5f5f5"
        : palette.BG};
`;
